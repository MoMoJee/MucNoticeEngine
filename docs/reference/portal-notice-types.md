...
2. 门户来源**必须支持翻页**（当前只抓第 1 页 20 条；推免名额在 type=6 第 2–3 页，名单在 type=11）。
3. 自动归档受时间窗限制（`max(2026-08-31, now - 90天)`），避免首跑爆炸；手动可绕过。
4. 实现细节见 [plans/0004](../plans/0004-portal-pagination-attachments.md)。
